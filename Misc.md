...
- 准备开户资料 https://mp.weixin.qq.com/s/qRCYYrYJri3IlHzb9p3xAg
...
- Q:
  - 1)如果手机可以通过如图 1 的 v2rayNG 上网了，那么手机开热点后，如何让连上热点的机器也能全局科学上网？
  - 2) 我的v2rayNG 怎么没有了选择 app 来上网的设置，以前的v2ray 我记得是有的
  - 3) 求推荐安卓下好用的 vmesh 协议的科学上网 app
  - 用一个淘汰的二手手机装上 v2rayNG 允许来自局域网的连接，然后让其他设备的全局代理设置成手机的 ip+v2rayNG 本地代理端口
- 设备选择
- 一台iPhone 或 Google Pixel
- 避免使用国产输入法 谷歌 Gboard 或苹果自带的输入法
- 避免使用国产浏览器和生态软件
- 避免使用国内邮箱和手机号注册境外软件 尽可能避免使用国内支付方式购买
- 使用一个独立的浏览器（如 Firefox），并为其创建一个专用的配置文件
- 最小化权限和追踪： 默认拒绝网站或 APP 的相册、通讯录、定位、麦克风、摄像头、蓝牙、通知等所有不必要的权限。  在浏览器设置中关闭第三方 Cookie 和跨站跟踪，禁止网页自动下载文件
- 密码管理器生成后再自己手动添加几个字符，并开启两步验证（2FA）
- 清理一次浏览器历史记录、Cookie、缓存和下载目录。
- 不要为国产苹果手机开启iCloud，不要用国区苹果ID登录国外苹果手机
