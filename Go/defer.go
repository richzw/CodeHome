// Source: https://juejin.im/post/5c964437e51d456d6d4fae46
...
// 控制台输出 3
复制代码传递引用给defer调用的函数，即使不使用闭包函数，值也会受后面的改变影响
...
// 控制台输出 5
复制代码传递值给defer调用的函数，且非闭包函数，值不会受后面的改变影响
func p(a int)  {
	fmt.Println(a)
}
...
// 控制台输出： 3
复制代码defer调用闭包函数，且内调用外部非传参进来的变量，值会受后面的改变影响
// 闭包函数内，事实是该值的引用
func defer3() {
	a := 3
	defer func() {
...
// 结果：数组越界错误
复制代码defer调用闭包函数，若内部使用了传参参数的值。使用的是值
func defer5() {
	a := []int{1,2,3}
	for i:=0;i<len(a);i++ {
...
//     2
//     1
复制代码defer所调用的非闭包函数，参数如果是函数，会按顺序先执行（函数参数）
...
得出于：www.cnblogs.com/qcrao-2018/…
