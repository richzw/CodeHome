...
int maxPoints(vector<Point> &pointvec) {
    if (pointvec.size() == 0)
    	return 0;
    ...
    for (int i = 0; i < pointvec.size(); ++i){
    	...
    	for (int j = 0; j < pointvec.size(); ++j){
    		...
    		if (pointvec[i].x == pointvec[j].x && pointvec[i].y == pointvec[j].y){
    			++same;
    		}else if (pointvec[i].x == pointvec[j].x && pointvec[i].y != pointvec[j].y){
    			++vertical_line;
    		}else{
    			float slope = (float)(pointvec[i].y - pointvec[j].y)/(float)(pointvec[i].x - pointvec[j].x);
    			map[slope]++;
   		...
   		imax = imax < max(cur_max+same, same+vertical_line)? max(cur_max+same, same+vertical_line): imax;
    }
